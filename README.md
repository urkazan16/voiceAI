...
```text
1. Capture   — hold Control+Shift+Space; the microphone is on only while held
2. Recognize — local Whisper ggml (SHA-256 + format check before use)
3. Format    — dictionary, snippets, backtrack, digits / dates / HH:MM, optional local LLM
4. Insert    — paste into the frontmost app, then restore the previous clipboard
```
...
- **Hold-to-talk** — default `Control+Shift+Space`. Hands-free mode is press-to-start / press-to-stop. Escape cancels the current take.
- **Local speech-to-text** — Whisper.cpp on CPU. Language: Russian, English, or auto-detect for mixed replicas. Default model is Whisper Medium (~1.5 GB), downloaded on first launch and verified before it is used.
- **Insert into other apps** — synthetic paste (`Cmd+V` on macOS, `Ctrl+V` elsewhere). The previous clipboard (text, RTF, images on macOS) is restored. Password fields are skipped; Copy last / Paste last still work after you leave the field.
- **Copy last, paste last, edit selection** — extra hotkeys (default `Cmd+Ctrl+C/V/E` on macOS, `Ctrl+Alt+C/V/E` elsewhere). Edit re-runs hold-to-talk and replaces the highlighted text.
- **Dictionary** — canonical terms plus aliases, and spoken → written replacement rules. Built-in developer vocabulary is seeded. Terms are also passed to Whisper as a decoding prompt.
- **Snippets** — exact triggers expand before the LLM (priority: command → snippet → dictionary).
- **Profiles** — per-app styles: `raw`, `normal`, `professional`, `code`. The frontmost app picks a profile; you can override it.
- **Personalization** — repeated corrections become suggestions; accepting one writes a dictionary rule.
- **Technical dictation** — hashes, GUIDs, domains, file names, versions, and spell-out mode (`по буквам` / `air bat cap`). See [Dictating technical text](#dictating-technical-text).
- **Numbers and dates** — spoken numbers as digits, DMY or ISO dates, clock times, list and punctuation voice commands, “scratch that” backtrack.
- **Repeat** — re-runs the last recording through the speech model currently in use (optional last-utterance WAV).
- **Model Manager** — download, checksum, activate, or delete Whisper and optional Qwen formatting models. Dictation works without an LLM.
- **History and journal** — SQLite history plus a JSONL utterance log; export, retry, turn a replica into a snippet.
- **Tray and LocalFlow Bar** — idle / recording / processing marks in the menu bar; optional floating bar while listening; start/end sound cues; launch at login.
- **Privacy** — audio, STT, dictionary, personalization, and history stay on disk under the data root. Network is only for user-initiated model download (and optional updates). Uninstall from the Privacy screen or `scripts/uninstall.sh` / `scripts/uninstall.ps1`.
- **Headless CLI** — transcribe a file, a directory, or stdin without opening the window.

UI language is English or Russian. The stack is Tauri 2 + Rust + React. This build is CPU-only.
...
| Host    | Avoid these system shortcuts                           | Paste                                                     |
| ------- | ------------------------------------------------------ | --------------------------------------------------------- |
...
Packaged builds: `.dmg` / `.app` (macOS), NSIS `.exe` (Windows, current user), `.deb` and AppImage (Linux).
...
drum each…`, or Russian `аз цап дэт ель…`) are preferred in a stream; letter
  ...
  open across push-to-talk presses until `конец` or ordinary speech.
