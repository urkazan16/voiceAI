...
- **Захват** — микрофон через `cpal`; поток открывается только на время записи и закрывается после отпускания, короткого удержания или отмены. Звук приводится к моно 16 кГц. Одна реплика не длиннее 20 минут (`MAX_CAPTURE_SECS`)
...
Поток микрофона открывается в момент нажатия клавиши и закрывается сразу после отпускания, отмены или сбоя захвата: вне записи LocalFlow ничего не слушает. Hands-free держит поток, пока сессия не остановлена, но буфер ограничен `MAX_CAPTURE_SECS` (20 мин), чтобы запись не росла без верхней границы.
