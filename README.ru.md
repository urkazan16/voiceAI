# LocalFlow — приватный голосовой ввод для Windows, macOS и Linux

[English](README.md) · **Русский**

LocalFlow — приложение с открытым исходным кодом для локального распознавания речи и голосового набора текста на Windows, macOS и Linux. После загрузки модели диктовка работает без интернета: зажал клавишу, сказал, отпустил — текст появился в активном поле любого приложения. Распознавание, оформление и вставка идут на вашем компьютере. Облачного аккаунта нет. Вставленный текст никогда не выполняется.
...
- Локальное преобразование речи в текст с Whisper после загрузки модели
- Диктовка в активное приложение по глобальной горячей клавише
- Русский и английский языки, словарь терминов и техническая диктовка
- Сниппеты, персонализация и необязательное оформление локальной LLM
- История диктовки и управление моделями в приложении

## Поддерживаемые платформы

| Платформа                        | Установщик           | Особенности                                              |
| -------------------------------- | -------------------- | -------------------------------------------------------- |
| Windows 10 / 11, x64             | NSIS `.exe`          | Требуется WebView2 Evergreen                             |
| macOS 12+, Apple Silicon / Intel | `.dmg`               | Нужны разрешения на микрофон и универсальный доступ      |
| Ubuntu 22.04+ / Linux, x64       | `.deb`, AppImage     | На Wayland автоматическая вставка зависит от композитора |
| Fedora Asahi Remix, aarch64      | Сборка из исходников | Поддерживаются 32-битный захват микрофона и Wayland      |
...
## Частые вопросы о голосовом вводе

### Можно ли диктовать текст без интернета?

Да. Сначала скачайте модель распознавания речи. Затем запись, распознавание, оформление и вставка работают локально. Для стилей с языковой моделью нужно заранее скачать и её веса.

### Как вводить текст голосом в браузере, мессенджере или IDE?

Поставьте курсор в текстовое поле, удерживайте Control+Shift+Space, произнесите фразу и отпустите клавиши. На Wayland может потребоваться ручная вставка из буфера обмена.

### Отправляются ли записи микрофона в облако?

Нет. Распознавание выполняется на компьютере. Сеть используется для загрузки моделей и необязательных обновлений приложения. История и последняя аудиозапись могут сохраняться локально — это управляется настройками.

### Есть ли распознавание русской и английской речи?

Да. Выберите `ru`, `en` или `auto`. Для переключения между языками используйте `auto`; целые предложения на разных языках внутри одной реплики могут распознаваться ненадёжно.

### Нужна ли локальная LLM для диктовки?

Нет. Режимы `raw` и `normal` работают без неё. Словарь, сниппеты и правила технической диктовки также не требуют LLM.
